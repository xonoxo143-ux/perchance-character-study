...
- 019 — Eragon Bromsson: accepted; pending merge into `research/SAMPLED_CHARACTERS.md`
- 020 — Clare Cooper: accepted; pending merge into `research/SAMPLED_CHARACTERS.md`

Note: `SAMPLED_CHARACTERS.md` is now large enough that a direct append via the GitHub contents tool hit fetch/truncation friction during this batch. This file preserves the accepted entries so they are not lost. The next cleanup pass should merge 019 and 020 into the main accepted ledger and then either keep this file as a batch note or remove it.
